...
PR #14's live dashboard cycle was verified in Chrome before this integration: four external weather responses, two completed OpenAI-controlled forecasts, 192 predictions matching the committed examples, and a saved 96-row CSV. The current combined-branch checks and candidate decision are recorded in [the integration review](docs/PR13_PR14_INTEGRATION.md). Earlier live-agent, recovery, full-February replay and fresh-environment evidence remains under [examples/submission](examples/submission/README.md). The trace in `examples/backend/recovery-events.jsonl` contains an explicitly labelled injected failure.
