...
- [x] First call publishes 96 rows for both turbines.
- [x] Second call advances the simulated clock by 12 hours and publishes a new version using a newer eligible weather run.
- [x] Revisions compare 36 overlapping target hours per turbine. Revision measures changed predictions, not prediction error.
- [x] Third call reports unchanged-input deduplication; forecasts remain at 192 rows and a skipped event is recorded.
- [x] Dashboard pointed at `outputs/final-agent` loads the new versions and trace. Refresh only reloads published files. Verified with Streamlit AppTest; browser checks remain with the teammate.
...
- [x] Include the [February day-ahead CSV](../examples/submission/february_day_ahead.csv): **1,344 unique rows, 672 February hours per turbine**, using horizons 25–48 from the preceding local midnight.
- [x] Include or link [verification and provenance](../examples/submission/README.md), measured January metrics, setup commands and known limitations.
- [x] Ensure the repository includes the portable offline example, source code and pinned requirements. Another machine must be able to run `python -m src demo` without a key or prior training.
- [x] Keep a separate local backup of the working `data/weather/`, `artifacts/` and generated `outputs/` directories for the demonstration. These ignored directories are not included by a Git push.
- [x] Exclude `.env`, API keys and the virtual environment from any package or backup shared with judges.
